...
words_in_poem = input('Введи стихотворение:').split()
vowels = ('аяоёиыуюеэ')
print(words_in_poem)
vowel_numbers = list(map(lambda word: sum(x in vowels for x in word), words_in_poem))  #  список количеств гласных в каждой фразе
for i in range(len(vowel_numbers)):
    if vowel_numbers[i] != vowel_numbers[0]:
        print('Пам парам')
        break
else:
    print('Парам пам-пам')
